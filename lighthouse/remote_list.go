...
type checkFuncV4 func(vpnIp netip.Addr, to *subnet.Ip4AndPort) bool
type checkFuncV6 func(vpnIp netip.Addr, to *subnet.Ip6AndPort) bool
...
type cacheV4 struct {
	learned  *subnet.Ip4AndPort
	reported []*subnet.Ip4AndPort
}

// cacheV4 stores learned and reported ipv6 records under cache
type cacheV6 struct {
	learned  *subnet.Ip6AndPort
	reported []*subnet.Ip6AndPort
}
...
func (r *RemoteList) unlockedSetLearnedV4(ownerVpnIp netip.Addr, to *subnet.Ip4AndPort) {
	r.shouldRebuild = true
...
func (r *RemoteList) unlockedSetV4(ownerVpnIp, vpnIp netip.Addr, to []*subnet.Ip4AndPort, check checkFuncV4) {
	r.shouldRebuild = true
...
func (r *RemoteList) unlockedPrependV4(ownerVpnIp netip.Addr, to *subnet.Ip4AndPort) {
	r.shouldRebuild = true
	c := r.unlockedGetOrMakeV4(ownerVpnIp)

	// We are doing the easy append because this is rarely called
	c.reported = append([]*subnet.Ip4AndPort{to}, c.reported...)
...
// deduplicated address list as dirty
func (r *RemoteList) unlockedSetLearnedV6(ownerVpnIp netip.Addr, to *subnet.Ip6AndPort) {
	r.shouldRebuild = true
...
// and marks the deduplicated address list as dirty
func (r *RemoteList) unlockedSetV6(ownerVpnIp, vpnIp netip.Addr, to []*subnet.Ip6AndPort, check checkFuncV6) {
	r.shouldRebuild = true
...
// This is only useful for establishing static hosts
func (r *RemoteList) unlockedPrependV6(ownerVpnIp netip.Addr, to *subnet.Ip6AndPort) {
	r.shouldRebuild = true
	c := r.unlockedGetOrMakeV6(ownerVpnIp)

	// We are doing the easy append because this is rarely called
	c.reported = append([]*subnet.Ip6AndPort{to}, c.reported...)
	if len(c.reported) > MaxRemotes {
		c.reported = c.reported[:MaxRemotes]
	}
}
