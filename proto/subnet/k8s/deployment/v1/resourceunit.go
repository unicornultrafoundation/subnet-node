...
func (r *ResourceUnit) totalResources() resourceLimits {
	...
	limits.cpu = utils.NewInt(limits.cpu.Int.Add(&limits.cpu.Int, &r.CPU.Units.Val.Int).Int64())
	limits.gpu = utils.NewInt(limits.gpu.Int.Add(&limits.gpu.Int, &r.GPU.Units.Val.Int).Int64())
	limits.memory = utils.NewInt(limits.memory.Int.Add(&limits.memory.Int, &r.Memory.Quantity.Val.Int).Int64())

	storage := make([]utils.BigInt, 0, len(r.Storage))
