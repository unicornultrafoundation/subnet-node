...
func (id OrderID) Validate() error {
	if err := id.GroupID().Validate(); err != nil {
		return errors.New("OrderID: Invalid GroupID")
	}
	if id.OSeq == 0 {
		return errors.New("OrderID: Invalid Order Sequence")
	}
	return nil
}
...
func MakeBidID(id OrderID, provider common.Address) BidID {
	return BidID{
		Owner:    id.Owner,
		DSeq:     id.DSeq,
		GSeq:     id.GSeq,
...
func (id BidID) Validate() error {
	if err := id.OrderID().Validate(); err != nil {
		return errors.New("BidID: Invalid OrderID")
	}
	if !common.IsHexAddress(id.Provider) {
		return errors.New("BidID: Invalid Provider Address")
	}
	if id.Owner == id.Provider {
		return errors.New("BidID: self-bid")
	}
	return nil
}
...
func (id LeaseID) Validate() error {
	if err := id.BidID().Validate(); err != nil {
		return errors.New("LeaseID: Invalid BidID")
	}
	return nil
}
