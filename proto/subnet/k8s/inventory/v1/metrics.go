...
func (inv *MetricTotal) AddResources(res dtypes.ResourceUnit) {
	cpu := utils.NewIntFromUint64(inv.CPU)
	gpu := utils.NewIntFromUint64(inv.GPU)
	mem := utils.NewIntFromUint64(inv.Memory)
	ephemeralStorage := utils.NewIntFromUint64(inv.StorageEphemeral)
	...
	for _, storage := range res.Storage {
		if storageClass, found := storage.Attributes.Find("class").AsString(); !found {
			...
			val := utils.NewIntFromUint64(uint64(inv.Storage[storageClass])) // nolint: gosec
