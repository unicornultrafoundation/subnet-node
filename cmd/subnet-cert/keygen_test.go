...
func Test_keygenSummary(t *testing.T) {
	assert.Equal(t, "keygen <flags>: create a public/private key pair. the public key can be passed to `subnet-cert sign`", keygenSummary())
}
...
func Test_keygenHelp(t *testing.T) {
	...
	assert.Equal(
		t,
		"Usage of "+os.Args[0]+" keygen <flags>: create a public/private key pair. the public key can be passed to `subnet-cert sign`\n"+
			"  -curve string\n"+
