...
func TestNewFirewallFromConfig(t *testing.T) {
	...
	_, err = NewFirewallFromConfig(l, conf, []netip.Prefix{})
	require.EqualError(t, err, "firewall.outbound rule #0; cidr must be provided")
...
func TestAddFirewallRulesFromConfig(t *testing.T) {
	...
	require.NoError(t, AddFirewallRulesFromConfig(l, true, conf, mf))
	assert.Equal(t, addRuleCall{incoming: true, proto: ProtoAny, startPort: 1, endPort: 1, groups: nil, ip: cidr}, mf.lastCall)
