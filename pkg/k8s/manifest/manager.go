...
import (
	...
	maniv2beta2 "github.com/unicornultrafoundation/subnet-node/proto/subnet/k8s/manifest/v1"
	mtypes "github.com/unicornultrafoundation/subnet-node/proto/subnet/k8s/market/v1"

	clustertypes "github.com/unicornultrafoundation/subnet-node/core/k8s/types/v1"
...
func newManager(h *service, daddr dtypes.DeploymentID) *manager {
	...
	m := &manager{
		...
		log:             session.Log().WithField("deployment", daddr).Logger,
		lc:              lifecycle.New(),
		config:          h.config,
...
func (m *manager) doFetchData(ctx context.Context) (manifestManagerFetchDataResult, error) {
	...
	// 	leases:     leases,
	// }, nil
	return manifestManagerFetchDataResult{}, nil
}
