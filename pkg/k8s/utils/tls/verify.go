...
type CertificateQuerier interface {
	GetAccountCertificate(context.Context, common.Address, *big.Int) (*x509.Certificate, crypto.PublicKey, error)
}
...
func ValidatePeerCertificates(
...
) (common.Address, *x509.Certificate, error) {
	if len(certs) == 0 {
		return common.Address{}, nil, CertificateInvalidError{nil, EmptyPeerCertificate}
	}
	...
	if !common.IsHexAddress(cert.Subject.CommonName) {
		return common.Address{}, nil, fmt.Errorf("%w: (%w)", CertificateInvalidError{cert, EmptyPeerCertificate}, err)
	}

	// 1. CommonName in issuer and Subject must match and be as Bech32 format
	if cert.Subject.CommonName != cert.Issuer.CommonName {
		return common.Address{}, nil, fmt.Errorf("%w: (%w)", CertificateInvalidError{cert, InvalidCN}, err)
	}

	// 2. serial number must be in
	if cert.SerialNumber == nil {
		return common.Address{}, nil, fmt.Errorf("%w: (%w)", CertificateInvalidError{cert, InvalidSN}, err)
	}

	// 3. look up the certificate on the chain
	onChainCert, _, err := cquery.GetAccountCertificate(ctx, owner, cert.SerialNumber)
	if err != nil {
		return common.Address{}, nil, fmt.Errorf("%w: (%w)", CertificateInvalidError{cert, Expired}, err)
	}
	...
	if _, err = cert.Verify(opts); err != nil {
		return common.Address{}, nil, fmt.Errorf("%w: (%w)", CertificateInvalidError{cert, Verify}, err)
	}
