	...
	for {
			...
			case *corev1.PersistentVolume:
				...
				case watch.Modified:
					...
					if !exists {
						...
						lblVal := scItem.Labels[builder.SubnetNodeManagedLabelName]
						if lblVal == "" {
							lblVal = falseVal
						}
						...
						params.isSubnetManaged, _ = strconv.ParseBool(lblVal)
			...
			evtdone:
				...
				case *storagev1.StorageClass:
					...
					case watch.Added:
						fallthrough
					case watch.Modified:
						lblVal := obj.Labels[builder.SubnetNodeManagedLabelName]
						if lblVal == "" {
							lblVal = falseVal
						}
		...
		case <-scrapech:
			...
			for class, params := range scs {
				if params.isRancher && params.isSubnetManaged {
