...
func isLabelManaged(key string) bool {
	return strings.HasPrefix(key, builder.SubnetNodeManagedLabelName) || key == labelNvidiaComGPUPresent
}
...
func generateLabels(cfg Config, knode *corev1.Node, node v1.Node, sc storageClasses) (map[string]string, v1.Node) {
	...
	res[builder.SubnetNodeManagedLabelName] = builder.ValTrue

	allowedSc := adjConfig.StorageClassesForNode(knode.Name)
	for _, class := range allowedSc {
		key := fmt.Sprintf("%s.class.%s", builder.SubnetNodeServiceCapabilityStorage, class)
		res[key] = "1"
	}
	...
	for _, info := range node.Resources.GPU.Info {
		...
		key := fmt.Sprintf("%s.vendor.%s.model.%s", builder.SubnetNodeServiceCapabilityGPU, info.Vendor, info.Name)
