...
import (
	...
	subnettypes "github.com/unicornultrafoundation/subnet-node/pkg/k8s/apis/subnet.node/v1"
	...
	ipoptypes "github.com/unicornultrafoundation/subnet-node/pkg/k8s/operator/ip/types"
...
func (op *ipOperator) recordEventError(ev cip.ResourceEvent, failure error) {
	...
	op.leasesIgnored.AddError(mtypes.LeaseIDToKey(ev.GetLeaseID()), failure, ev.GetSharingKey())
	op.flagIgnoredLeases()
}
...
func (op *ipOperator) applyEvent(ctx context.Context, ev cip.ResourceEvent) error {
	...
	case ctypes.ProviderResourceAdd, ctypes.ProviderResourceUpdate:
		if op.leasesIgnored.IsFlagged(mtypes.LeaseIDToKey(ev.GetLeaseID())) {
...
func (op *ipOperator) applyAddOrUpdateEvent(ctx context.Context, ev cip.ResourceEvent) error {
	...
	op.log.
		...
		WithField("service", ev.GetServiceName()).
		WithField("externalPort", ev.GetExternalPort()).
		Info("connecting")
	...
	if isSameLease {
		if !exists {
			...
			op.log.
				WithField("lease", leaseID).
				Info("ip passthrough is new, applying")
			...
			if hasChanged {
				...
				op.log.
					WithField("lease", leaseID).
					Info("ip passthrough has changed, applying")
			}
		}
...
func handleHTTPError(op *ipOperator, rw http.ResponseWriter, req *http.Request, err error, status int) {
	...
	if err != nil {
		op.log.
			WithError(err).
			Error("failed writing response body")
	}
}

func newIPOperator(ctx context.Context, logger *logrus.Logger, ns string, cfg common.OperatorConfig, ilc common.IgnoreListConfig, mllbc metallb.Client) (*ipOperator, error) {
	...
	retval.server.GetRouter().Use(func(next http.Handler) http.Handler {
		return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
			if !retval.barrier.enter() {
				retval.log.
					WithField("path", req.URL.Path).
					Error("barrier is locked, can't service request")
...
func handleIPLeaseStatusGet(op *ipOperator, rw http.ResponseWriter, req *http.Request) {
	...
	if !ecommon.IsHexAddress(owner) {
		op.log.Error("could not parse owner address as bech32", "onwer", owner)
		rw.WriteHeader(http.StatusNotFound)
		return
	}
	...
	if err != nil {
		op.log.
			WithError(err).
			Error("failed writing JSON of ip status response")
	}
}
...
// nolint: unused
func (op *ipOperator) getDeclaredIPs(ctx context.Context, leaseID mtypes.LeaseID) ([]subnettypes.ProviderLeasedIPSpec, error) {
	labelSelector := &strings.Builder{}
	kubeSelectorForLease(labelSelector, leaseID)

	results, err := op.ac.SubnetV1().ProviderLeasedIPs(op.ns).List(ctx, metav1.ListOptions{
	...
	retval := make([]subnettypes.ProviderLeasedIPSpec, 0, len(results.Items))
...
func (op *ipOperator) PurgeDeclaredIP(ctx context.Context, leaseID mtypes.LeaseID, serviceName string, externalPort uint32, proto manifest.ServiceProtocol) error {
	...
	return op.ac.SubnetV1().ProviderLeasedIPs(op.ns).DeleteCollection(ctx, metav1.DeleteOptions{}, metav1.ListOptions{
...
func (op *ipOperator) DeclareIP(ctx context.Context, lID mtypes.LeaseID, serviceName string, port uint32, externalPort uint32, proto manifest.ServiceProtocol, sharingKey string, overwrite bool) error {
	...
	op.log.
		WithField("resource-name", resourceName).
		Debug("checking for resource")
	foundEntry, err := op.ac.SubnetV1().ProviderLeasedIPs(op.ns).Get(ctx, resourceName, metav1.GetOptions{})
	exists := true
	if err != nil {
	...
	obj := subnettypes.ProviderLeasedIP{
		ObjectMeta: metav1.ObjectMeta{
			Name:   resourceName,
			Labels: labels,
		},
		Spec: subnettypes.ProviderLeasedIPSpec{
			LeaseID:      subnettypes.LeaseIDFromSubnet(lID),
	...
	if exists {
		...
		_, err = op.ac.SubnetV1().ProviderLeasedIPs(op.ns).Update(ctx, &obj, metav1.UpdateOptions{})
	} else {
		_, err = op.ac.SubnetV1().ProviderLeasedIPs(op.ns).Create(ctx, &obj, metav1.CreateOptions{})
	}
...
func (op *ipOperator) purgeDeclaredIPs(ctx context.Context, lID mtypes.LeaseID) error {
	labelSelector := &strings.Builder{}
	_, err := fmt.Fprintf(labelSelector, "%s=true,", builder.SubnetNodeManagedLabelName)
	if err != nil {
		return err
	}
	kubeSelectorForLease(labelSelector, lID)
	result := op.ac.SubnetV1().ProviderLeasedIPs(op.ns).DeleteCollection(ctx, metav1.DeleteOptions{}, metav1.ListOptions{
		LabelSelector: labelSelector.String(),
	})
		...
		for {
			...
			case <-ctx.Done():
				...
				if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
					op.log.
						WithError(err).
						Error("failed watching metal LB config map changes")
				}
				return
			case ev, ok := <-watcher.ResultChan():
				...
				op.log.
					WithField("event-type", ev.Type).
					Debug("metal LB config change event")
				output <- struct{}{}
			}
		}
	}()
...
func (op *ipOperator) observeIPState(ctx context.Context) (<-chan cip.ResourceEvent, error) {
	...
	phpager := pager.New(func(ctx context.Context, opts metav1.ListOptions) (runtime.Object, error) {
		resources, err := op.ac.SubnetV1().ProviderLeasedIPs(op.ns).List(ctx, opts)
	...
	data := make([]subnettypes.ProviderLeasedIP, 0, 128)
	err := phpager.EachListItem(ctx, metav1.ListOptions{}, func(obj runtime.Object) error {
		plip := obj.(*subnettypes.ProviderLeasedIP)
	...
	op.log.
		WithField("resource-version", lastResourceVersion).
		Info("starting ip passthrough watch")
	watcher, err := op.ac.SubnetV1().ProviderLeasedIPs(op.ns).Watch(ctx, metav1.ListOptions{
	...
	for i, v := range data {
		if !ecommon.IsHexAddress(v.Spec.LeaseID.Owner) {
			return nil, fmt.Errorf("invalid owner address")
		}
		ownerAddr := ecommon.HexToAddress(v.Spec.LeaseID.Owner)
		if !ecommon.IsHexAddress(v.Spec.LeaseID.Provider) {
			return nil, fmt.Errorf("invalid provider address")
		}
		providerAddr := ecommon.HexToAddress(v.Spec.LeaseID.Provider)
	...
	go func() {
		...
		for {
			...
			case result, ok := <-results:
				...
				plip := result.Object.(*subnettypes.ProviderLeasedIP)
				if !ecommon.IsHexAddress(plip.Spec.LeaseID.Owner) {
					op.log.
						WithField("addr", plip.Spec.LeaseID.Owner).
						Error("invalid owner address in provider host")
					continue // Ignore event
				}
				ownerAddr := ecommon.HexToAddress(plip.Spec.LeaseID.Owner)
				if !ecommon.IsHexAddress(plip.Spec.LeaseID.Provider) {
					op.log.
						WithField("addr", plip.Spec.LeaseID.Provider).
						Error("invalid provider address in provider host")
					continue // Ignore event
				}
				providerAddr := ecommon.HexToAddress(plip.Spec.LeaseID.Provider)
