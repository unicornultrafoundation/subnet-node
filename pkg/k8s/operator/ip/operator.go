...
func handleIPLeaseStatusGet(op *ipOperator, rw http.ResponseWriter, req *http.Request) {
	...
	owner := vars["owner"]
	// Validate this is a bech32 address
	if !caddr.IsHexAddress(owner) {
...
func (op *ipOperator) observeIPState(ctx context.Context) (<-chan cip.ResourceEvent, error) {
	...
	for i, v := range data {
		if !caddr.IsHexAddress(v.Spec.LeaseID.Owner) {
			return nil, fmt.Errorf("invalid owner address in provider host: %s", v.Spec.LeaseID.Owner)
		}
		if !caddr.IsHexAddress(v.Spec.LeaseID.Provider) {
			return nil, fmt.Errorf("invalid provider address in provider host: %s", v.Spec.LeaseID.Provider)
		}

		ownerAddr := caddr.HexToAddress(v.Spec.LeaseID.Owner)
		providerAddr := caddr.HexToAddress(v.Spec.LeaseID.Provider)
	...
	go func() {
		...
		for {
			...
			case result, ok := <-results:
				...
				if !caddr.IsHexAddress(plip.Spec.LeaseID.Owner) {
					op.log.Error("invalid owner address in provider host", "addr", plip.Spec.LeaseID.Owner, "err", err)
					continue
				}
				if !caddr.IsHexAddress(plip.Spec.LeaseID.Provider) {
					op.log.Error("invalid provider address in provider host", "addr", plip.Spec.LeaseID.Provider, "err", err)
					continue
				}

				ownerAddr := caddr.HexToAddress(plip.Spec.LeaseID.Owner)
				providerAddr := caddr.HexToAddress(plip.Spec.LeaseID.Provider)
