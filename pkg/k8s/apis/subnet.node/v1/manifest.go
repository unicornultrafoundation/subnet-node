...
import (
	...
	ktypes "github.com/unicornultrafoundation/subnet-node/core/k8s/kube/types/v1"
...
// FromCRD returns subnet group details formatted from manifest group
...
// manifestGroupToCRD returns manifest group instance from subnet group
...
func manifestServiceFromProvider(ams mani.Service, schedulerParams *SchedulerParams) (ManifestService, error) {
	resources, err := resourcesFromSubnet(ams.Resources)
	if err != nil {
	...
	for _, expose := range ams.Expose {
		ms.Expose = append(ms.Expose, manifestServiceExposeFromSubnet(expose))
	}
...
func (mse ManifestServiceExpose) toSubnet() (mani.ServiceExpose, error) {
...
func manifestServiceExposeFromSubnet(amse mani.ServiceExpose) ManifestServiceExpose {
