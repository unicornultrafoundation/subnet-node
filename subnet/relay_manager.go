...
func (rm *relayManager) EstablishRelay(relayHostInfo *HostInfo, m *subnetp.SubnetControl) (*Relay, error) {
...
func (rm *relayManager) HandleControlMsg(h *HostInfo, m *subnetp.SubnetControl, f *Interface) {

	switch m.Type {
	case subnetp.SubnetControl_CreateRelayRequest:
		rm.handleCreateRelayRequest(h, f, m)
	case subnetp.SubnetControl_CreateRelayResponse:
		rm.handleCreateRelayResponse(h, f, m)
	}

}

func (rm *relayManager) handleCreateRelayResponse(h *HostInfo, f *Interface, m *subnetp.SubnetControl) {
	...
	if peerRelay.State == PeerRequested {
		...
		resp := subnetp.SubnetControl{
			Type:                subnetp.SubnetControl_CreateRelayResponse,
...
func (rm *relayManager) handleCreateRelayRequest(h *HostInfo, f *Interface, m *subnetp.SubnetControl) {
	//TODO: IPV6-WORK
	b := [4]byte{}
	...
	if target == f.myVpnNet.Addr() {
		...
		if sendCreateRequest {
			...
			req := subnetp.SubnetControl{
				Type:                subnetp.SubnetControl_CreateRelayRequest,
