...
func TestRemoteList_Rebuild(t *testing.T) {
	...
	rl.unlockedSetV6(
		...
		func(netip.Addr, *subnetp.Ip6AndPort) bool { return true },
	)
...
func newIp4AndPortFromString(s string) *subnetp.Ip4AndPort {
	a := netip.MustParseAddrPort(s)
	v4Addr := a.Addr().As4()
	return &subnetp.Ip4AndPort{
		Ip:   binary.BigEndian.Uint32(v4Addr[:]),
		Port: uint32(a.Port()),
	}
}

func newIp6AndPortFromString(s string) *subnetp.Ip6AndPort {
	a := netip.MustParseAddrPort(s)
	v6Addr := a.Addr().As16()
	return &subnetp.Ip6AndPort{
