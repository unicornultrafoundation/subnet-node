...
func newLighthouseMetrics() *MessageMetrics {
	gen := func(t string) [][]metrics.Counter {
		h := make([][]metrics.Counter, len(subnetp.SubnetMeta_MessageType_name))
		used := []subnetp.SubnetMeta_MessageType{
			subnetp.SubnetMeta_HostQuery,
			subnetp.SubnetMeta_HostQueryReply,
			subnetp.SubnetMeta_HostUpdateNotification,
			subnetp.SubnetMeta_HostPunchNotification,
			subnetp.SubnetMeta_HostUpdateNotificationAck,
		}
		for _, i := range used {
