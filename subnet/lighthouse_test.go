...
func TestNewLhQuery(t *testing.T) {
	...
	assert.IsType(t, &subnetp.SubnetMeta{}, a)
...
func BenchmarkLighthouseHandleRequest(b *testing.B) {
	...
	lh.addrMap[vpnIp3].unlockedSetV4(
		...
		func(netip.Addr, *subnetp.Ip4AndPort) bool { return true },
	)
	...
	b.Run("notfound", func(b *testing.B) {
		...
		req := &subnetp.SubnetMeta{
			Type: subnetp.SubnetMeta_HostQuery,
			Details: &subnetp.SubnetMetaDetails{
				VpnIp:       4,
				Ip4AndPorts: nil,
			},
		}
...
func newLHHostRequest(fromAddr netip.AddrPort, myVpnIp, queryVpnIp netip.Addr, lhh *LightHouseHandler) testLhReply {
	...
	filter := subnetp.SubnetMeta_HostQueryReply
	w := &testEncWriter{
		metaFilter: &filter,
	}
...
func newLHHostUpdate(fromAddr netip.AddrPort, vpnIp netip.Addr, addrs []netip.AddrPort, lhh *LightHouseHandler) {
	...
	bip := vpnIp.As4()
	req := &subnetp.SubnetMeta{
		Type: subnetp.SubnetMeta_HostUpdateNotification,
		Details: &subnetp.SubnetMetaDetails{
			VpnIp:       binary.BigEndian.Uint32(bip[:]),
			Ip4AndPorts: make([]*subnetp.Ip4AndPort, len(addrs)),
		},
	}
...
type testLhReply struct {
	...
	vpnIp      netip.Addr
	msg        *subnetp.SubnetMeta
}

type testEncWriter struct {
	lastReply  testLhReply
	metaFilter *subnetp.SubnetMeta_MessageType
}
...
func assertIp4InArray(t *testing.T, have []*subnetp.Ip4AndPort, want ...netip.AddrPort) {
