...
func (f *Interface) readOutsidePackets(ip netip.AddrPort, via *ViaSender, out []byte, packet []byte, h *header.H, fwPacket *firewall.Packet, lhf udp.LightHouseHandlerFunc, nb []byte, q int, localCache firewall.ConntrackCache) {
	...
	case header.Message:
		...
		case header.MessageRelay:
			...
			// The packet consists of a 16-byte parsed Subnet header, Associated Data-protected payload, and a trailing 16-byte AEAD signature value.
...
/*
func (f *Interface) sendMeta(ci *ConnectionState, endpoint *net.UDPAddr, meta *SubnetMeta) {
	if ci.eKey != nil {
		//TODO: log error?
		return
	}
