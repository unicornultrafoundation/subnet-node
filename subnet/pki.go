...
func (p *PKI) reloadCert(c *config.C, initial bool) *util.ContextualError {
	...
	p.cs.Store(cs)
	if initial {
		p.l.WithField("cert", cs.Certificate).Debug("Client subnet certificate")
	} else {
...
func newCertState(certificate cert.Certificate, pkcs11backed bool, privateKey []byte) (*CertState, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invalid subnet certificate on interface: %s", err)
	}
...
func loadPrivateKey(privPathOrPEM string) (rawKey []byte, curve certp.Curve, isPkcs11 bool, err error) {
	...
	if strings.Contains(privPathOrPEM, "-----BEGIN") {
		...
		return rawKey, certp.Curve_P256, true, nil
	} else {
...
func newCertStateFromConfig(c *config.C) (*CertState, error) {
	...
	subnetCert, _, err := cert.UnmarshalCertificateFromPEM(rawCert)
	if err != nil {
		return nil, fmt.Errorf("error while unmarshaling pki.cert %s: %s", pubPathOrPEM, err)
	}

	if subnetCert.Expired(time.Now()) {
		return nil, fmt.Errorf("subnet certificate for this host is expired")
	}

	if len(subnetCert.Networks()) == 0 {
		return nil, fmt.Errorf("no networks encoded in certificate")
	}

	if err = subnetCert.VerifyPrivateKey(curve, rawKey); err != nil {
		return nil, fmt.Errorf("private key is not a pair with public key in subnet cert")
	}

	return newCertState(subnetCert, isPkcs11, rawKey)
}
