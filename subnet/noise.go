...
type SubnetCipherState struct {
	c noise.Cipher
	//k [32]byte
	//n uint64
}

func NewSubnetCipherState(s *noise.CipherState) *SubnetCipherState {
	return &SubnetCipherState{c: s.Cipher()}
...
func (s *SubnetCipherState) EncryptDanger(out, ad, plaintext []byte, n uint64, nb []byte) ([]byte, error) {
	if s != nil {
...
func (s *SubnetCipherState) DecryptDanger(out, ad, ciphertext []byte, n uint64, nb []byte) ([]byte, error) {
	if s != nil {
		nb[0] = 0
		nb[1] = 0
		nb[2] = 0
...
func (s *SubnetCipherState) Overhead() int {
	if s != nil {
