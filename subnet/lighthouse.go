...
import (
	...
	"github.com/unicornultrafoundation/subnet-node/core/udp"
	"github.com/unicornultrafoundation/subnet-node/proto/subnet"
	subnetp "github.com/unicornultrafoundation/subnet-node/proto/subnet"
)
...
type LightHouse struct {
	...
	ifce         EncWriter
	subnetPort   uint32 // 32 bits because protobuf does not have a uint16
...
func NewLightHouseFromConfig(ctx context.Context, l *logrus.Logger, c *config.C, myVpnNet netip.Prefix, pc udp.Conn, p *Punchy) (*LightHouse, error) {
	...
	subnetPort := uint32(c.GetInt("listen.port", 0))
	if amLighthouse && subnetPort == 0 {
	...
	if subnetPort == 0 && pc != nil {
		uPort, err := pc.LocalAddr()
		if err != nil {
			return nil, util.NewContextualError("Failed to get listening port", nil, err)
		}
		subnetPort = uint32(uPort.Port())
	}
		...
		for i, rawAddr := range rawAdvAddrs {
			...
			if lh.myVpnNet.Contains(ip) {
				lh.l.WithField("addr", rawAddr).WithField("entry", i+1).
					Warn("Ignoring lighthouse.advertise_addrs report because it is within the subnet network range")
				continue
			}
...
func (lh *LightHouse) addCalculatedRemotes(vpnIp netip.Addr) bool {
	...
	var calculated []*subnetp.Ip4AndPort
...
func (lh *LightHouse) unlockedShouldAddV4(vpnIp netip.Addr, to *subnet.Ip4AndPort) bool {
...
func (lh *LightHouse) unlockedShouldAddV6(vpnIp netip.Addr, to *subnet.Ip6AndPort) bool {
...
func lhIp6ToIp(v *subnet.Ip6AndPort) net.IP {
	ip := make(net.IP, 16)
...
func NewLhQueryByInt(vpnIp netip.Addr) *subnet.SubnetMeta {
	if vpnIp.Is6() {
	...
	b := vpnIp.As4()
	return &subnet.SubnetMeta{
		Type: subnet.SubnetMeta_HostQuery,
		Details: &subnet.SubnetMetaDetails{
			VpnIp: binary.BigEndian.Uint32(b[:]),
		},
	}
}

func AddrPortFromIp4AndPort(ip *subnet.Ip4AndPort) netip.AddrPort {
	b := [4]byte{}
	binary.BigEndian.PutUint32(b[:], ip.Ip)
	return netip.AddrPortFrom(netip.AddrFrom4(b), uint16(ip.Port))
}

func AddrPortFromIp6AndPort(ip *subnet.Ip6AndPort) netip.AddrPort {
	b := [16]byte{}
...
func NewIp4AndPortFromNetIP(ip netip.Addr, port uint16) *subnet.Ip4AndPort {
	v4Addr := ip.As4()
	return &subnet.Ip4AndPort{
...
func NewIp6AndPortFromNetIP(ip netip.Addr, port uint16) *subnet.Ip6AndPort {
	ip6Addr := ip.As16()
	return &subnet.Ip6AndPort{
...
func (lh *LightHouse) innerQueryServer(ip netip.Addr, nb, out []byte) {
	...
	lh.metricTx(subnet.SubnetMeta_HostQuery, int64(len(lighthouses)))
...
func (lh *LightHouse) SendUpdate() {
	var v4 []*subnet.Ip4AndPort
	var v6 []*subnet.Ip6AndPort
	...
	for _, e := range localIps(lh.l, lal) {
		...
		if e.Is4() {
			v4 = append(v4, NewIp4AndPortFromNetIP(e, uint16(lh.subnetPort)))
		} else {
			v6 = append(v6, NewIp6AndPortFromNetIP(e, uint16(lh.subnetPort)))
		}
	}
	...
	m := &subnet.SubnetMeta{
		Type: subnet.SubnetMeta_HostUpdateNotification,
		Details: &subnet.SubnetMetaDetails{
			VpnIp:       binary.BigEndian.Uint32(b[:]),
			Ip4AndPorts: v4,
			Ip6AndPorts: v6,
	...
	lighthouses := lh.GetLighthouses()
	lh.metricTx(subnet.SubnetMeta_HostUpdateNotification, int64(len(lighthouses)))
	nb := make([]byte, 12, 12)
	out := make([]byte, mtu)
...
func (lh *LightHouse) metricRx(t subnet.SubnetMeta_MessageType, i int64) {
	lh.metrics.Rx(header.MessageType(t), 0, i)
}

func (lh *LightHouse) metricTx(t subnet.SubnetMeta_MessageType, i int64) {
...
func (lhh *LightHouseHandler) resetMeta() *subnet.SubnetMeta {
...
func (lhh *LightHouseHandler) HandleRequest(rAddr netip.AddrPort, vpnIp netip.Addr, p []byte, w EncWriter) {
	...
	switch n.Type {
	case subnet.SubnetMeta_HostQuery:
		lhh.handleHostQuery(n, vpnIp, rAddr, w)

	case subnet.SubnetMeta_HostQueryReply:
		lhh.handleHostQueryReply(n, vpnIp)

	case subnet.SubnetMeta_HostUpdateNotification:
		lhh.handleHostUpdateNotification(n, vpnIp, w)

	case subnet.SubnetMeta_HostMovedNotification:
	case subnet.SubnetMeta_HostPunchNotification:
		lhh.handleHostPunchNotification(n, vpnIp, w)

	case subnet.SubnetMeta_HostUpdateNotificationAck:
		// noop
	}
}

func (lhh *LightHouseHandler) handleHostQuery(n *subnet.SubnetMeta, vpnIp netip.Addr, addr netip.AddrPort, w EncWriter) {
	...
	found, ln, err := lhh.lh.queryAndPrepMessage(queryVpnIp, func(c *cache) (int, error) {
		n = lhh.resetMeta()
		n.Type = subnet.SubnetMeta_HostQueryReply
	...
	lhh.lh.metricTx(subnet.SubnetMeta_HostQueryReply, 1)
	...
	found, ln, err = lhh.lh.queryAndPrepMessage(vpnIp, func(c *cache) (int, error) {
		n = lhh.resetMeta()
		n.Type = subnet.SubnetMeta_HostPunchNotification
		//TODO: IPV6-WORK
		b = vpnIp.As4()
	...
	lhh.lh.metricTx(subnet.SubnetMeta_HostPunchNotification, 1)
...
func (lhh *LightHouseHandler) coalesceAnswers(c *cache, n *subnet.SubnetMeta) {
	if c.v4 != nil {
		if c.v4.learned != nil {
...
func (lhh *LightHouseHandler) handleHostQueryReply(n *subnet.SubnetMeta, vpnIp netip.Addr) {
...
func (lhh *LightHouseHandler) handleHostUpdateNotification(n *subnet.SubnetMeta, vpnIp netip.Addr, w EncWriter) {
	if !lhh.lh.amLighthouse {
		if lhh.l.Level >= logrus.DebugLevel {
	...
	n = lhh.resetMeta()
	n.Type = subnet.SubnetMeta_HostUpdateNotificationAck
	...
	lhh.lh.metricTx(subnet.SubnetMeta_HostUpdateNotificationAck, 1)
	w.SendMessageToVpnIp(header.LightHouse, 0, vpnIp, lhh.pb[:ln], lhh.nb, lhh.out[:0])
}

func (lhh *LightHouseHandler) handleHostPunchNotification(n *subnet.SubnetMeta, vpnIp netip.Addr, w EncWriter) {
	if !lhh.lh.IsLighthouseIP(vpnIp) {
		return
	}
	...
	// This sends a subnet test packet to the host trying to contact us. In the case
	...
	if lhh.lh.punchy.GetRespond() {
		...
		go func() {
			...
			if lhh.l.Level >= logrus.DebugLevel {
				lhh.l.Debugf("Sending a subnet test packet to vpn ip %s", queryVpnIp)
			}
