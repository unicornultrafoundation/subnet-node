...
import (
	...
	"github.com/unicornultrafoundation/subnet-node/crypto/hash"
)
...
func (sp *Proof) ValidateBasic() error {
	...
	if len(sp.LeafHash) != hash.Size {
		return fmt.Errorf("expected LeafHash size to be %d, got %d", hash.Size, len(sp.LeafHash))
	}
	...
	for i, auntHash := range sp.Aunts {
		if len(auntHash) != hash.Size {
			return fmt.Errorf("expected Aunts#%d size to be %d, got %d", i, hash.Size, len(auntHash))
		}
	}
	return nil
}
