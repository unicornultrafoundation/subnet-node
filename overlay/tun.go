...
func getAllRoutesFromConfig(c *config.C, vpnNetworks []netip.Prefix, initial bool) (bool, []Route, error) {
	...
	if err != nil {
		return true, nil, cerror.NewContextualError("Could not parse tun.routes", nil, err)
	}

	unsafeRoutes, err := parseUnsafeRoutes(c, vpnNetworks)
	if err != nil {
		return true, nil, cerror.NewContextualError("Could not parse tun.unsafe_routes", nil, err)
	}
