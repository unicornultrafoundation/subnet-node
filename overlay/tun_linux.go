...
import (
	...
	cerror "github.com/unicornultrafoundation/subnet-node/common/error"
...
func newTunGeneric(c *config.C, l *logrus.Logger, file *os.File, vpnNetworks []netip.Prefix) (*tun, error) {
	...
	c.RegisterReloadCallback(func(c *config.C) {
		...
		if err != nil {
			cerror.LogWithContextIfNeeded("failed to reload tun device", err, t.l)
		}
	})
...
func (t *tun) reload(c *config.C, initial bool) error {
	...
	if !initial {
		...
		if err != nil {
			...
			cerror.LogWithContextIfNeeded("Failed to refresh routes", err, t.l)
		}
	}
...
func (t *tun) addRoutes(logErrors bool) error {
	...
	for _, r := range routes {
		...
		err := netlink.RouteReplace(&nr)
		if err != nil {
			retErr := cerror.NewContextualError("Failed to add route", map[string]any{"route": r}, err)
			if logErrors {
				retErr.Log(t.l)
			} else {
				return retErr
