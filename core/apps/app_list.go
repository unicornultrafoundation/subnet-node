...
func (s *Service) GetApps(ctx context.Context, start *big.Int, end *big.Int, filter AppFilter) ([]*App, int, error) {
	...
	for i, subnetApp := range subnetApps {
		...
		if (filter.Status == "" || appStatus == filter.Status) &&
			...
			apps = append(apps, convertToApp(subnetApp, appId, appStatus))
		}
	}
