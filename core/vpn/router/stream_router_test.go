...
func TestStreamRouter_GetOrCreateRoute(t *testing.T) {
	// Create mocks
	mockCreator := new(MockStreamCreator)
	...
	// Setup mock for CreateNewVPNStream
	mockStream := new(MockVPNStream)
	mockCreator.On("CreateNewVPNStream", mock.Anything, mock.Anything).Return(mockStream, nil)

	// Create a wrapper function for the mock's CreateNewVPNStream method
	createStreamFn := func(ctx context.Context, peerID peer.ID) (api.VPNStream, error) {
		return mockCreator.CreateNewVPNStream(ctx, peerID)
	}

	// Create router with the wrapper function
	router := NewStreamRouter(config, createStreamFn, mockPeerDiscovery)
