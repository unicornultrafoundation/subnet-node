...
func New(cfg *config.C) *VPNConfig {
	...
	return &VPNConfig{
		...
		PacketBufferSize:    cfg.GetInt("vpn.packet_buffer_size", 50),                              // 500 packets buffer size
		...
		UsageCountThreshold: cfg.GetInt("vpn.usage_count_threshold", 100),                          // 100 default threshold for usage count
