...
func (t *tun) Activate() error {
	...
	/*
		...
		if err = ioctl(fd, unix.SIOCSIFTXQLEN, uintptr(unsafe.Pointer(&ifrq))); err != nil {
			// If we can't set the queue length subnet will still work but it may lead to packet loss
...
func (t *tun) reload(c *config.C, initial bool) error {
	...
	// Teach subnet how to handle the routes before establishing them in the system table
