...
type TestTun struct {
	...
	closed    atomic.Bool
	rxPackets chan []byte // Packets to receive into subnet
	TxPackets chan []byte // Packets transmitted outside by subnet
}
...
// Send will place a byte array onto the receive queue for subnet to consume
// These are unencrypted ip layer frames destined for another subnet node.
...
// subnet meant to send this message to some application on the local system
