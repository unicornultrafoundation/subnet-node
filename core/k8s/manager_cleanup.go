package k8s

import (
	"context"

	"github.com/sirupsen/logrus"
	v2beta2 "github.com/unicornultrafoundation/subnet-node/proto/subnet/k8s/manifest/v1"
	mtypes "github.com/unicornultrafoundation/subnet-node/proto/subnet/k8s/market/v1"
)

type purgeIPEntry struct {
	serviceName string
	port        uint32
	proto       v2beta2.ServiceProtocol
}

type deployCleanupHelper struct {
	client Client
	log    *logrus.Logger
	lease  mtypes.LeaseID

	hostnamesToPurge []string
	ipsToPurge       []purgeIPEntry
}

func newDeployCleanupHelper(lease mtypes.LeaseID, client Client, log *logrus.Logger) *deployCleanupHelper {
	return &deployCleanupHelper{
		client: client,
		log:    log,
		lease:  lease,
	}
}

func (dch *deployCleanupHelper) addHostname(hostname string) {
	dch.hostnamesToPurge = append(dch.hostnamesToPurge, hostname)
}

func (dch *deployCleanupHelper) addIP(serviceName string, port uint32, proto v2beta2.ServiceProtocol) {
	dch.ipsToPurge = append(
		dch.ipsToPurge,
		purgeIPEntry{
			serviceName: serviceName,
			port:        port,
			proto:       proto,
		})
}

func (dch *deployCleanupHelper) purgeAll(ctx context.Context) {
	// for _, hostname := range dch.hostnamesToPurge {
	// 	err := dch.client.PurgeDeclaredHostname(ctx, dch.lease, hostname)
	// 	if err != nil {
	// 		dch.log.WithField("lease", dch.lease).WithField("hostname", hostname).WithError(err).Error("could not purge hostname")
	// 	}
	// }

	// for _, ipEntry := range dch.ipsToPurge {
	// 	err := dch.client.PurgeDeclaredIP(ctx, dch.lease, ipEntry.serviceName, ipEntry.port, ipEntry.proto)
	// 	if err != nil {
	// 		dch.log.WithField("lease", dch.lease).WithField("serviceName", ipEntry.serviceName).WithField("port", ipEntry.port).WithField("proto", ipEntry.proto).WithError(err).Error("could not purge IP")
	// 	}
	// }
}
