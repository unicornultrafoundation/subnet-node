...
func NewClient(ctx context.Context, logger *logrus.Logger, poolName string, endpoint *net.SRV) (Client, error) {
	...
	return &client{
		...
		poolName: poolName,
		l:        &sync.Mutex{},
		log:      logger.WithField("client", "metallb").Logger,
	}, nil
...
func (c *client) GetIPAddressStatusForLease(ctx context.Context, leaseID mtypes.LeaseID) ([]cip.LeaseState, error) {
	...
	_, err = fmt.Fprintf(labelSelector, ",%s=%s", builder.SubnetNodeServiceTarget, builder.SubnetNodeMetalLB)
	if err != nil {
		return nil, err
	}
	...
	err = servicePager.EachListItem(ctx, metav1.ListOptions{
		...
		func(obj runtime.Object) error {
			...
			selectedServiceName := service.Spec.Selector[builder.SubnetNodeManifestServiceLabelName]
...
func (c *client) CreateIPPassthrough(ctx context.Context, directive cip.ClusterIPPassthroughDirective) error {
	...
	labels[builder.SubnetNodeServiceTarget] = builder.SubnetNodeMetalLB

	selector := map[string]string{
		builder.SubnetNodeManagedLabelName:         "true",
		builder.SubnetNodeManifestServiceLabelName: directive.ServiceName,
	}
...
func (c *client) GetIPPassthroughs(ctx context.Context) ([]cip.Passthrough, error) {
	...
	err = servicePager.EachListItem(ctx,
		...
		func(obj runtime.Object) error {
			...
			_, hasOwner := service.ObjectMeta.Labels[builder.SubnetNodeLeaseOwnerLabelName]
			if !hasOwner {
			...
			serviceName := serviceSelector[builder.SubnetNodeManifestServiceLabelName]
