...
/* A client to talk to the K8s Subnet Node implementation of the IP Operator via HTTP */
type client struct {
...
func NewClient(ctx context.Context, logger *logrus.Logger, endpoint *net.SRV) (cip.Client, error) {
	sda, err := clusterutil.NewServiceDiscoveryAgent(ctx, logger, "rest", "operator-ip", "subnet-services", endpoint)
	if err != nil {
		return nil, err
	}
	...
	return &client{
		sda: sda,
		log: logger.WithField("operator", "ip").Logger,
		l:   &sync.Mutex{},
	}, nil
}
