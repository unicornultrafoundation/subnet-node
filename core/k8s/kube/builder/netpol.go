...
func (b *netPol) Create() ([]*netv1.NetworkPolicy, error) { // nolint:golint,unparam
	...
	result := []*netv1.NetworkPolicy{
		{
			...
			ObjectMeta: metav1.ObjectMeta{
				Name:      SubnetNodeDeploymentPolicyName,
				Labels:    b.labels(),
	...
	for _, service := range b.deployment.ManifestGroup().Services {
		...
		if len(ports) != 0 {
			...
			policyName := fmt.Sprintf("subnet-node-%s-np", serviceName)
			policy := netv1.NetworkPolicy{
				...
				Spec: netv1.NetworkPolicySpec{
					...
					PodSelector: metav1.LabelSelector{
						MatchLabels: map[string]string{
							SubnetNodeManifestServiceLabelName: serviceName,
						},
					},
					PolicyTypes: []netv1.PolicyType{
						netv1.PolicyTypeIngress,
		...
		if len(portsWithIP) != 0 {
			...
			policyName := fmt.Sprintf("subnet-node-%s-ip", serviceName)
			policy := netv1.NetworkPolicy{
				ObjectMeta: metav1.ObjectMeta{
					Labels:    b.labels(),
					Name:      policyName,
...
func (b *netPol) Update(obj *netv1.NetworkPolicy) (*netv1.NetworkPolicy, error) { // nolint:golint,unparam
	...
	uobj.Labels = updateSubnetLabels(obj.Labels, b.labels())
