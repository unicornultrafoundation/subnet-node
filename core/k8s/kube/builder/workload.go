...
func (b *Workload) affinity() *corev1.Affinity {
	...
	selectors := []corev1.NodeSelectorRequirement{
		{
			Key:      SubnetNodeManagedLabelName,
	...
	for _, storage := range service.Resources.Storage {
		...
		if class, valid := attr.AsString(); valid {
			selectors = append(selectors, corev1.NodeSelectorRequirement{
				Key:      fmt.Sprintf("%s.class.%s", SubnetNodeServiceCapabilityStorage, class),
...
func nodeSelectorsFromResources(res *crd.SchedulerResources) []corev1.NodeSelectorRequirement {
	...
	if gpu := res.GPU; gpu != nil {
		key := fmt.Sprintf("%s.vendor.%s.model.%s", SubnetNodeServiceCapabilityGPU, gpu.Vendor, gpu.Model)
...
func (b *Workload) labels() map[string]string {
	...
	obj[SubnetNodeManifestServiceLabelName] = b.deployment.ManifestGroup().Services[b.serviceIdx].Name
...
func (b *Workload) addEnvVarsForDeployment(envVarsAlreadyAdded map[string]int, env []corev1.EnvVar) []corev1.EnvVar {
	...
	env = addIfNotPresent(envVarsAlreadyAdded, env, envVarSubnetNodeGroupSequence, lid.GetGSeq())
	env = addIfNotPresent(envVarsAlreadyAdded, env, envVarSubnetNodeDeploymentSequence, lid.GetDSeq())
	env = addIfNotPresent(envVarsAlreadyAdded, env, envVarSubnetNodeOrderSequence, lid.GetOSeq())
	env = addIfNotPresent(envVarsAlreadyAdded, env, envVarSubnetNodeOwner, lid.Owner)
	env = addIfNotPresent(envVarsAlreadyAdded, env, envVarSubnetNodeProvider, lid.Provider)
	env = addIfNotPresent(envVarsAlreadyAdded, env, envVarSubnetNodeClusterPublicHostname, b.settings.ClusterPublicHostname)
