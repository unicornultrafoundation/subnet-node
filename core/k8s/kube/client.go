...
import (
	...
	mapi "github.com/unicornultrafoundation/subnet-node/proto/subnet/k8s/manifest/v1"
	...
	cluster "github.com/unicornultrafoundation/subnet-node/core/k8s"
	...
	subnetclient "github.com/unicornultrafoundation/subnet-node/pkg/k8s/client/clientset/versioned"
...
func NewClient(ctx context.Context, log *logrus.Logger, ns string, virtualIP string) (Client, error) {
	...
	log = log.WithField("client", "kube").Logger
...
func (c *client) GetDeployment(ctx context.Context, dID dtypes.DeploymentID) ([]ctypes.IDeployment, error) {
	...
	_, _ = fmt.Fprintf(labelSelectors, "%s=%d", builder.SubnetNodeLeaseDSeqLabelName, dID.DSeq)
	_, _ = fmt.Fprintf(labelSelectors, ",%s=%s", builder.SubnetNodeLeaseOwnerLabelName, dID.Owner)

	manifests, err := wrapKubeCall("manifests-list", func() (*crd.ManifestList, error) {
		return c.ac.SubnetV1().Manifests(c.ns).List(ctx, metav1.ListOptions{
...
func (c *client) GetManifestGroup(ctx context.Context, lID mtypes.LeaseID) (bool, crd.ManifestGroup, error) {
	...
	obj, err := wrapKubeCall("manifests-get", func() (*crd.Manifest, error) {
		return c.ac.SubnetV1().Manifests(c.ns).Get(ctx, leaseNamespace, metav1.GetOptions{})
	})
...
func (p *previousObj) recover(ctx context.Context, kc kubernetes.Interface, ac subnetclient.Interface) []error {
	...
	if p.omani != nil {
		if _, err := ac.SubnetV1().Manifests(p.omani.Namespace).Update(ctx, p.omani, metav1.UpdateOptions{}); err != nil {
			errs = append(errs, err)
		}
	}
...
func (c *client) Deploy(ctx context.Context, deployment ctypes.IDeployment) (err error) {
	...
	po.omani, err = c.ac.SubnetV1().Manifests(c.ns).Get(ctx, applies.cmanifest.Name(), metav1.GetOptions{})
	...
	if cdeployment.UpdateManifest() {
		...
		if po.omani != nil {
			if !reflect.DeepEqual(&po.umani.Spec, &po.omani.Spec) || !reflect.DeepEqual(po.umani.Labels, po.omani.Labels) {
				po.umani, err = c.ac.SubnetV1().Manifests(applies.cmanifest.NS()).Update(ctx, po.umani, metav1.UpdateOptions{})
			}
		} else {
			po.nmani, err = c.ac.SubnetV1().Manifests(applies.cmanifest.NS()).Create(ctx, po.nmani, metav1.CreateOptions{})
		}
...
func (c *client) TeardownLease(ctx context.Context, lid mtypes.LeaseID) error {
	...
	_, err := wrapKubeCall("manifests-delete", func() (interface{}, error) {
		return nil, c.ac.SubnetV1().Manifests(c.ns).Delete(ctx, builder.LidNS(lid), metav1.DeleteOptions{})
	})
...
func (c *client) LeaseEvents(ctx context.Context, lid mtypes.LeaseID, services string, follow bool) (ctypes.EventsWatcher, error) {
	...
	if len(services) != 0 {
		listOpts.LabelSelector = fmt.Sprintf(builder.SubnetNodeManifestServiceLabelName+" in (%s)", services)
	}
				...
				for _, port := range service.Spec.Ports {
					...
					if nodePort > 0 {
						...
						case corev1.ProtocolUDP:
							v.Proto = mapi.UDP
						default:
							isValid = false // Skip this, since the Protocol is set to something not supported by Subnet
						}
						if isValid {
...
func (c *client) ServiceStatus(ctx context.Context, lid mtypes.LeaseID, name string) (*apclient.ServiceStatus, error) {
	...
	mani, err := wrapKubeCall("manifests-list", func() (*crd.Manifest, error) {
		return c.ac.SubnetV1().Manifests(c.ns).Get(ctx, builder.LidNS(lid), metav1.GetOptions{})
	})
