...
func (c *client) GetDeclaredIPs(ctx context.Context, leaseID mtypes.LeaseID) ([]subnettypes.ProviderLeasedIPSpec, error) {
	labelSelector := &strings.Builder{}
	kubeSelectorForLease(labelSelector, leaseID)

	results, err := c.ac.SubnetV1().ProviderLeasedIPs(c.ns).List(ctx, metav1.ListOptions{
...
func (c *client) PurgeDeclaredIP(ctx context.Context, leaseID mtypes.LeaseID, serviceName string, externalPort uint32, proto manifest.ServiceProtocol) error {
	...
	return c.ac.SubnetV1().ProviderLeasedIPs(c.ns).DeleteCollection(ctx, metav1.DeleteOptions{}, metav1.ListOptions{
...
func (c *client) DeclareIP(ctx context.Context, lID mtypes.LeaseID, serviceName string, port uint32, externalPort uint32, proto manifest.ServiceProtocol, sharingKey string, overwrite bool) error {
	...
	foundEntry, err := c.ac.SubnetV1().ProviderLeasedIPs(c.ns).Get(ctx, resourceName, metav1.GetOptions{})
	exists := true
	if err != nil {
...
func (c *client) PurgeDeclaredIPs(ctx context.Context, lID mtypes.LeaseID) error {
	...
	result := c.ac.SubnetV1().ProviderLeasedIPs(c.ns).DeleteCollection(ctx, metav1.DeleteOptions{}, metav1.ListOptions{
		LabelSelector: labelSelector.String(),
	})
...
func (c *client) ObserveIPState(ctx context.Context) (<-chan cip.ResourceEvent, error) {
	...
	phpager := pager.New(func(ctx context.Context, opts metav1.ListOptions) (runtime.Object, error) {
		resources, err := c.ac.SubnetV1().ProviderLeasedIPs(c.ns).List(ctx, opts)
	...
	watcher, err := c.ac.SubnetV1().ProviderLeasedIPs(c.ns).Watch(ctx, metav1.ListOptions{
	...
	for i, v := range data {
		if !common.IsHexAddress(v.Spec.LeaseID.Owner) {
			c.log.Error("invalid owner address in provider host", "addr", v.Spec.LeaseID.Owner)
			continue
		}
		if !common.IsHexAddress(v.Spec.LeaseID.Provider) {
			c.log.Error("invalid provider address in provider host", "addr", v.Spec.LeaseID.Provider)
			continue
		}
	...
	go func() {
		...
		for {
			...
			case result, ok := <-results:
				...
				if !common.IsHexAddress(plip.Spec.LeaseID.Owner) {
					c.log.Error("invalid owner address in provider host", "addr", plip.Spec.LeaseID.Owner)
					continue
				}
				if !common.IsHexAddress(plip.Spec.LeaseID.Provider) {
					c.log.Error("invalid provider address in provider host", "addr", plip.Spec.LeaseID.Provider)
					continue
				}
