...
import (
	...
	subnetclient "github.com/unicornultrafoundation/subnet-node/pkg/k8s/client/clientset/versioned/fake"
)
...
type execScaffold struct {
	...
	subnetFake *subnetclient.Clientset
...
func withExecTestScaffold(t *testing.T, changePod func(pod *corev1.Pod) error, test func(s *execScaffold)) {
	...
	s.subnetFake = subnetclient.NewClientset(s.crdManifest)
	...
	pod := &corev1.Pod{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"subnet.node/manifest-service": execTestServiceName,
			},
		},
	...
	s.client = &client{
		...
		ac:  s.subnetFake,
		ns:  testKubeClientNs,
		log: myLog.WithField("mode", "test-kube-provider-client").Logger,
...
func TestClientExecWrongServiceName(t *testing.T) {
	withExecTestScaffold(t, nil, func(s *execScaffold) {
		...
		_, err := s.client.Exec(s.ctx,
			...
			false,
			nil)
		require.ErrorIs(t, err, k8s.ErrExecNoServiceWithName)
	})
}
...
func TestClientExecWrongPodIndex(t *testing.T) {
	withExecTestScaffold(t, nil, func(s *execScaffold) {
		...
		_, err := s.client.Exec(s.ctx,
			...
			false,
			nil)
		require.ErrorIs(t, err, k8s.ErrExecPodIndexOutOfRange)
	})
}
...
func TestClientExecPodNotRunning(t *testing.T) {
	withExecTestScaffold(t, func(pod *corev1.Pod) error {
		...
		_, err := s.client.Exec(s.ctx,
			...
			false,
			nil)
		require.ErrorIs(t, err, k8s.ErrExecServiceNotRunning)
