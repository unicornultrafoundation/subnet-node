...
func (hID hostnameID) Equals(other hostnameID) bool {
	return hID.gseq == other.gseq &&
		hID.dseq == other.dseq &&
		strings.ToLower(hID.owner.String()) == strings.ToLower(other.owner.String())
}
...
func prepareHostnamesImpl(store map[string]hostnameID, hostnames []string, hID hostnameID, errCh chan<- error) {
	...
	for _, hostname := range hostnames {
		...
		if ok {
			if strings.ToLower(existingID.owner.String()) == strings.ToLower(hID.owner.String()) {
...
func (sh *SimpleHostnames) CanReserveHostnames(hostnames []string, ownerAddr common.Address) error {
	sh.lock.Lock()
	defer sh.lock.Unlock()
...
func canReserveHostnamesImpl(store map[string]hostnameID, hostnames []string, ownerAddr common.Address, chErr chan<- error) {
	for _, hostname := range hostnames {
		existingID, inUse := store[hostname]

		if inUse {
			if strings.ToLower(existingID.owner.String()) != strings.ToLower(ownerAddr.String()) {
...
func (hs *hostnameService) CanReserveHostnames(hostnames []string, ownerAddr common.Address) error {
