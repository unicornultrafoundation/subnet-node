...
func NewServiceDiscoveryAgent(ctx context.Context, logger *logrus.Logger, portName, serviceName, namespace string, endpoint *net.SRV) (ServiceDiscoveryAgent, error) {
	...
	sda := &serviceDiscoveryAgent{
		...
		pendingRequests: nil,
		result:          nil,
		log:             logger.WithField("cmp", "service-discovery-agent").Logger,
	}
	go sda.run()
