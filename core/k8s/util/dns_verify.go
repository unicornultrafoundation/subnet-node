...
func VerifyDNSVerification(ctx context.Context, hostname, token string, timeout time.Duration) (verified bool, message string) {
	...
	if err != nil {
		...
		if errors.As(err, &dnsErr) {
			if dnsErr.IsNotFound {
				return false, fmt.Sprintf("DNS TXT record not found: please add a TXT record '%s.%s' with value %q in your DNS provider", subnetPrefixVerify, hostname, token)
			}
			if dnsErr.IsTimeout {
	...
	if len(txtRecords) == 0 {
		return false, fmt.Sprintf("no TXT records found: please add a TXT record '%s.%s' with value %q in your DNS provider", subnetPrefixVerify, hostname, token)
	}
	...
	return false, fmt.Sprintf("verification token mismatch: expected %q, but found TXT record(s): %v. Please update your DNS TXT record '%s.%s' to contain exactly: %q", token, txtRecords, subnetPrefixVerify, hostname, token)
}
