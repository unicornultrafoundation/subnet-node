...
func TestMarshalingSubnetCertificate(t *testing.T) {
...
//func TestSubnetCertificate_Sign(t *testing.T) {
//	before := time.Now().Add(time.Second * -60).Round(time.Second)
//	after := time.Now().Add(time.Second * 60).Round(time.Second)
//	pubKey := []byte("1234567890abcedfghij1234567890ab")
//
...
//func TestSubnetCertificate_SignP256(t *testing.T) {
//	before := time.Now().Add(time.Second * -60).Round(time.Second)
//	after := time.Now().Add(time.Second * 60).Round(time.Second)
...
func TestSubnetCertificate_Expired(t *testing.T) {
	nc := certificateV1{
		details: detailsV1{
...
func TestSubnetCertificate_MarshalJSON(t *testing.T) {
	time.Local = time.UTC
	pubKey := []byte("1234567890abcedfghij1234567890ab")
...
func TestSubnetCertificate_Verify(t *testing.T) {
...
func TestSubnetCertificate_VerifyP256(t *testing.T) {
...
func TestSubnetCertificate_Verify_IPs(t *testing.T) {
...
func TestSubnetCertificate_Verify_Subnets(t *testing.T) {
	caIp1 := mustParsePrefixUnmapped("10.0.0.0/16")
	caIp2 := mustParsePrefixUnmapped("192.168.0.0/24")
...
func TestSubnetCertificate_VerifyPrivateKey(t *testing.T) {
	ca, _, caKey, err := newTestCaCert(time.Time{}, time.Time{}, nil, nil, nil)
	assert.Nil(t, err)
	err = ca.VerifyPrivateKey(certp.Curve_CURVE25519, caKey)
	assert.Nil(t, err)

	_, _, caKey2, err := newTestCaCert(time.Time{}, time.Time{}, nil, nil, nil)
	assert.Nil(t, err)
	err = ca.VerifyPrivateKey(certp.Curve_CURVE25519, caKey2)
	assert.NotNil(t, err)

	c, _, priv, err := newTestCert(ca, caKey, time.Time{}, time.Time{}, nil, nil, nil)
	err = c.VerifyPrivateKey(certp.Curve_CURVE25519, priv)
	assert.Nil(t, err)

	_, priv2 := x25519Keypair()
	err = c.VerifyPrivateKey(certp.Curve_CURVE25519, priv2)
	assert.NotNil(t, err)
}

func TestSubnetCertificate_VerifyPrivateKeyP256(t *testing.T) {
	ca, _, caKey, err := newTestCaCertP256(time.Time{}, time.Time{}, nil, nil, nil)
	assert.Nil(t, err)
	err = ca.VerifyPrivateKey(certp.Curve_P256, caKey)
	assert.Nil(t, err)

	_, _, caKey2, err := newTestCaCertP256(time.Time{}, time.Time{}, nil, nil, nil)
	assert.Nil(t, err)
	err = ca.VerifyPrivateKey(certp.Curve_P256, caKey2)
	assert.NotNil(t, err)

	c, _, priv, err := newTestCert(ca, caKey, time.Time{}, time.Time{}, nil, nil, nil)
	err = c.VerifyPrivateKey(certp.Curve_P256, priv)
	assert.Nil(t, err)

	_, priv2 := p256Keypair()
	err = c.VerifyPrivateKey(certp.Curve_P256, priv2)
	assert.NotNil(t, err)
}
...
//func TestMarshalingSubnetCertificateConsistency(t *testing.T) {
...
func TestSubnetCertificate_Copy(t *testing.T) {
	ca, _, caKey, err := newTestCaCert(time.Now(), time.Now().Add(10*time.Minute), nil, nil, nil)
	assert.Nil(t, err)
...
func TestUnmarshalSubnetCertificate(t *testing.T) {
...
func newTestCaCert(before, after time.Time, ips, subnets []netip.Prefix, groups []string) (Certificate, []byte, []byte, error) {
	...
	nc, err := tbs.Sign(nil, certp.Curve_CURVE25519, priv)
	if err != nil {
...
func newTestCaCertP256(before, after time.Time, ips, subnets []netip.Prefix, groups []string) (Certificate, []byte, []byte, error) {
	...
	nc, err := tbs.Sign(nil, certp.Curve_P256, rawPriv)
	if err != nil {
		return nil, nil, nil, err
	}
...
func newTestCert(ca Certificate, key []byte, before, after time.Time, ips, subnets []netip.Prefix, groups []string) (Certificate, []byte, []byte, error) {
	...
	switch ca.Curve() {
	case certp.Curve_CURVE25519:
		pub, rawPriv = x25519Keypair()
	case certp.Curve_P256:
