...
type SubnetEncryptedData struct {
	EncryptionMetadata SubnetEncryptionMetadata
	Ciphertext         []byte
}

type SubnetEncryptionMetadata struct {
...
func EncryptAndMarshalSigningPrivateKey(curve certp.Curve, b []byte, passphrase []byte, kdfParams *Argon2Parameters) ([]byte, error) {
	ciphertext, err := aes256Encrypt(passphrase, kdfParams, b)
	if err != nil {
		return nil, err
	}

	b, err = proto.Marshal(&certp.RawSubnetEncryptedData{
		EncryptionMetadata: &certp.RawSubnetEncryptionMetadata{
			EncryptionAlgorithm: "AES-256-GCM",
			Argon2Parameters: &certp.RawSubnetArgon2Parameters{
...
// UnmarshalSubnetEncryptedData will unmarshal a protobuf byte representation of a subnet cert into its
// protobuf-generated struct.
func UnmarshalSubnetEncryptedData(b []byte) (*SubnetEncryptedData, error) {
	if len(b) == 0 {
		return nil, fmt.Errorf("nil byte array")
	}
	var rned certp.RawSubnetEncryptedData
...
func unmarshalArgon2Parameters(params *certp.RawSubnetArgon2Parameters) (*Argon2Parameters, error) {
...
func DecryptAndUnmarshalSigningPrivateKey(passphrase, b []byte) (certp.Curve, []byte, []byte, error) {
	...
	default:
		return curve, nil, r, fmt.Errorf("bytes did not contain a proper subnet encrypted Ed25519/ECDSA private key banner")
	}

	ned, err := UnmarshalSubnetEncryptedData(k.Bytes)
	if err != nil {
