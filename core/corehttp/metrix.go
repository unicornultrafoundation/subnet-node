...
func MetricsOpenCensusCollectionOption() ServeOption {
	return func(_ *core.SubnetNode, _ net.Listener, mux *http.ServeMux) (*http.ServeMux, error) {
...
func MetricsOpenCensusDefaultPrometheusRegistry() ServeOption {
	return func(_ *core.SubnetNode, _ net.Listener, mux *http.ServeMux) (*http.ServeMux, error) {
		...
		pe, err := ocprom.NewExporter(ocprom.Options{
			...
			OnError: func(err error) {
				log.Errorln("OC default registry ERROR", "error", err)
			},
		})
		if err != nil {
			return nil, err
...
type SubnetNodeCollector struct {
	Node *core.SubnetNode
}

func (SubnetNodeCollector) Describe(ch chan<- *prometheus.Desc) {
	ch <- peersTotalMetric
}

func (c SubnetNodeCollector) Collect(ch chan<- prometheus.Metric) {
...
func (c SubnetNodeCollector) PeersTotalValues() map[string]float64 {
